...
const Portfolio = ({ portfolioItems }) => {
    ...
    const handleClick = (tagName) => {
        ...
        if (tagName === "alles" && router.asPath !== "/") {
            setGridItems(portfolioItems);
        } else if (tagName === "alles" && router.asPath === "/") {
            setGridItems(portfolioItems.slice(1, 9));
        } else {
            setGridItems(
                portfolioItems.filter((item) => {
                    if (item.tagList.length >= 2) {
                        // => Image has Multiple tags
                        return item.tagList.find((el) => el.title === tagName);
                    } else {
                        // => Image has just One tags
    ...
    return (
        <>
                                    ...
                                    <span
                                        className={
                                            tag.name === "eigen werk"
                                                ...
                                                ? `hidden sm:inline-block absolute top-8 border-l-[20px] translate-x-[-160%] border-l-transparent border-t-[7px] border-t-accent border-r-[20px] border-r-transparent`
                                                : tag.name === "editorial"
            ...
            <div className="max-w-7xl px-5 sm:px-10 pt-10 mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-12">
                {" "}
                {/* => Show Grid Items */}
                {allGridItems}
            </div>
